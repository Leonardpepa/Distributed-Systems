import src.bank.view.client_protocol as client_protocol

from connection_handler import ConnectionHandler

def client_loop(connection_handler):
    authenticated = False
    uid = ""
    id = -1
    name = ""
    ...
    while not authenticated:
        ...
        if user_input == "1":
            ...
            ok, result = client_protocol.handleLogin(connection_handler)
            if ok:
                id = result["id"]
                uid = result["uid"]
                name = result["name"]
        ...
        if user_input == "2":
            ...
            ok, result = client_protocol.handleRegister(connection_handler)
            if ok:
                id = result["id"]
                uid = result["uid"]
                name = result["name"]
    ...
    while authenticated:
        ...
        if user_input == "1":
            ...
            client_protocol.handleInfo(connection_handler, uid)
            # print("--------------------")
            
        if user_input == "2":
            print("----STATEMENT INFO----", flush=True)
            client_protocol.get_statements(connection_handler, uid)
            # print("--------------------")
        ...
        if user_input == "3":
            ...
            client_protocol.handleDeposit(connection_handler, uid)
            # print("---------------")
            
        # Withdraw routine
        if user_input == "4":
            print("----WITHDRAW----", flush=True)
            client_protocol.handleWithdraw(connection_handler, uid)
        
        # Balance routine
        if user_input == "5":
            print("----BALANCE----", flush=True)
            client_protocol.handleBalance(connection_handler, uid)  
            # print("---------------")
        
        if user_input == "6":
            print("----TRANFER----", flush=True)
            client_protocol.handleTransfer(connection_handler, uid, id)
            # print("---------------")
            
        if user_input == "7":
            print("----LOGOUT----", flush=True)
            ok, result = client_protocol.logout(connection_handler, uid)
            if ok:
                authenticated = False
                id = -1
                name = ""
            print(result["message"]) 
            return client_loop(connection_handler=connection_handler)



def main():
    connection_handler = ConnectionHandler()
    client_loop(connection_handler)            
